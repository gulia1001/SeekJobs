...
from app.components.llm_client import ClaudeRecommendationClient, GrokClient
...
def render(bundle: dict[str, object]) -> None:
    ...
    with left:
        ...
        projects_certs = st.text_area("Projects and Certificates", placeholder="List your key projects and certifications here...")
        analyze = st.button("🔍 Analyze resume", use_container_width=True)
        generate_cv = st.button("📄 Generate AI CV (LaTeX)", use_container_width=True)
    ...
    with right:
        ...
        if generate_cv:
            st.subheader("🤖 AI Agent: CV Generation")
            grok = GrokClient()
            if not grok.is_configured:
                st.error("Groq API key not found in .env. CV generation is disabled.")
            else:
                with st.spinner("Generating tailored LaTeX CV..."):
                    top_matches_list = []
                    ...
                    profile_with_links = resume_profile.copy()
                    profile_with_links["linkedin_url"] = linkedin_url
                    profile_with_links["github_url"] = github_url
                    profile_with_links["projects_certs"] = projects_certs
                    
                    cv_latex = grok.generate_cv_latex(profile_with_links, top_matches_list)
                    st.text_area("Generated LaTeX CV", value=cv_latex, height=400)
                    st.download_button(
                    ...
                    st.info("Copy the LaTeX code above and compile it in your favorite LaTeX editor (like Overleaf) to get a professional PDF.")
    ...
    with tabs[2]:
        if matches.empty:
            ...
            for _, row in matches.iterrows():
                ...
                with st.expander("📄 View Vacancy Details"):
                    if "description_clean_v2" in row and pd.notna(row["description_clean_v2"]) and row["description_clean_v2"]:
                        st.markdown(row["description_clean_v2"])
                    elif "description" in row and pd.notna(row["description"]) and row["description"]:
                        st.markdown(row["description"])
                    elif "requirements_clean" in row and pd.notna(row["requirements_clean"]) and row["requirements_clean"]:
                        st.markdown(row["requirements_clean"])
                    else:
                        st.write("No detailed description available for this vacancy.")
